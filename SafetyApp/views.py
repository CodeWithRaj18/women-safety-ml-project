...
def RegisterAction(request):
    if request.method == 'POST':
        ...
        if output == "none":
            db_connection = pymysql.connect(host='127.0.0.1', port=3306,
                                            user='root', password='',
                                            database='safety', charset='utf8')
            db_cursor = db_connection.cursor()

            sql = """
            INSERT INTO register(username,password,mobile,email,address)
            VALUES(%s,%s,%s,%s,%s)
            """

            db_cursor.execute(sql, (username, password, contact, email, address))
